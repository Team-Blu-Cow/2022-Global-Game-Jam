...
namespace blu
{
    ...
    public class SceneModule : Module
    {
        ...
        private float InitTransitionType(TransitionType in_transitionType)
        {
            ...
            switch (in_transitionType)
            {
                ...
                case TransitionType.LRSweep:
                    _transitionPrefab = Instantiate(Resources.Load<GameObject>("UI/Scene Transitions/Prefab/SceneTransition_ L_R Sweep")); // instantiates the LR Sweep prefab canvas
                ...
                case TransitionType.Fade:
                    _transitionPrefab = Instantiate(Resources.Load<GameObject>("UI/Scene Transitions/Prefab/SceneTransition_ Fade To Black")); // instantiates the LR Sweep prefab canvas
                    DontDestroyOnLoad(_transitionPrefab); // stops the scene loader from destroying it after switch
                    _transitionAnimator = _transitionPrefab.GetComponentInChildren<Animator>();
                    in_delay = 0.5f;
                    break;
                ...
                case TransitionType.Slice:
                    _transitionPrefab = Instantiate(Resources.Load<GameObject>("UI/Scene Transitions/Prefab/SceneTransition_ Slice")); // instantiates the LR Sweep prefab canvas
                    DontDestroyOnLoad(_transitionPrefab); // stops the scene loader from destroying it after switch
                    _transitionAnimator = _transitionPrefab.GetComponentInChildren<Animator>();
                    in_delay = 0.5f;
                    break;
        ...
        private void InitLoadingBar(LoadingBarType in_loadingBarType)
        {
            switch (in_loadingBarType)
            {
                ...
                case LoadingBarType.BottomRightRadial:
                    _progressBarPrefab = Instantiate(Resources.Load<GameObject>("UI/Scene Transitions/Prefab/SceneProgress_ Radial"));
                ...
                case LoadingBarType.BottomBar:
                    _progressBarPrefab = Instantiate(Resources.Load<GameObject>("UI/Scene Transitions/Prefab/SceneProgress_ Line"));
                    DontDestroyOnLoad(_progressBarPrefab);
                    _progressBar = _progressBarPrefab.GetComponentInChildren<ProgressBar>();
                    break;
        ...
        {
            App.GetModule<InputModule>().Input.Disable();
            ...
            App.GetModule<InputModule>().Input.Enable();
